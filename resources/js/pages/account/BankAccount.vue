...
<template>
    ...
    <div class="space-y-3 px-3 pb-24 pt-3">
        <AccountPageHeader title="Liên kết ngân hàng" description="Thông tin dùng để nạp và rút tiền" />
        ...
        <form class="space-y-3 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm" @submit.prevent="submit">
            <div
                class="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] text-amber-800">
                <Landmark class="size-4 shrink-0" />
                <span>Đảm bảo thông tin chính xác. Tên chủ tài khoản phải trùng khớp với tên đăng ký tại ngân
                    hàng.</span>
            </div>
            ...
            <div>
                ...
                <Select v-model="form.bank_name">
                    <SelectTrigger id="bank_name" class="account-select-trigger">
                        <SelectValue placeholder="Chọn ngân hàng" />
                    </SelectTrigger>
                    <SelectContent class="account-select-content">
                        <SelectItem
            ...
            <div>
                ...
                <input id="bank_account_number" :value="form.bank_account_number" type="text" inputmode="numeric"
                    autocomplete="off" maxlength="32" class="account-input font-mono tracking-wider"
                    :class="{ 'is-invalid': form.errors.bank_account_number }" @input="onAccountNumberInput" />
                <p v-if="form.errors.bank_account_number" class="account-error">{{ form.errors.bank_account_number }}
                </p>
            </div>

            <div>
                <label for="bank_account_name" class="account-label">Tên chủ tài khoản</label>
                <input id="bank_account_name" v-model="form.bank_account_name" type="text" autocomplete="off"
                    maxlength="160" class="account-input uppercase"
...
/* Select styled to match .account-input */
:deep(.account-select-trigger) {
    width: 100%;
    height: 2.75rem !important;
    padding: 0 0.875rem !important;
    border: 1.5px solid rgb(231 229 228) !important;
...
:deep(.account-select-trigger:hover:not(:disabled)) {
    border-color: rgb(214 211 209) !important;
}

:deep(.account-select-trigger:focus),
:deep(.account-select-trigger:focus-visible),
:deep(.account-select-trigger[data-state='open']) {
    border-color: rgb(217 119 6) !important;
    box-shadow: 0 0 0 3px rgb(254 243 199) !important;
}

:deep(.account-select-trigger[data-placeholder]) {
    color: rgb(168 162 158) !important;
}

:deep(.account-select-trigger svg) {
