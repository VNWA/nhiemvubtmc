...
const creditRows = computed(() =>
    props.bySource.filter((r) => r.direction === 'credit').sort((a, b) => b.total_vnd - a.total_vnd),
);
const debitRows = computed(() =>
    props.bySource.filter((r) => r.direction === 'debit').sort((a, b) => b.total_vnd - a.total_vnd),
);
...
<template>
    ...
    <div class="space-y-3 px-3 pb-24 pt-3">
        <AccountPageHeader
            ...
            description="Tổng hợp dòng tiền của bạn"
        />
        ...
        <section class="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-2 grid grid-cols-2 gap-2">
                <div class="rounded-xl bg-emerald-50 p-2.5">
                    ...
                    <p class="mt-0.5 font-mono text-base font-bold text-emerald-800">+{{ formatVnd(totals.totalCreditVnd) }}</p>
                </div>
                <div class="rounded-xl bg-rose-50 p-2.5">
                    ...
                    <p class="mt-0.5 font-mono text-base font-bold text-rose-800">-{{ formatVnd(totals.totalDebitVnd) }}</p>
                </div>
            </div>
        ...
        <section class="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
            ...
            <div class="mt-2 grid grid-cols-3 gap-2 text-center">
                <div class="rounded-xl bg-emerald-50 p-2">
                    ...
                    <p class="mt-0.5 font-mono text-xs font-bold text-emerald-800">+{{ formatVnd(last30Days.totalCreditVnd) }}</p>
                </div>
                <div class="rounded-xl bg-rose-50 p-2">
                    <p class="text-[10px] font-semibold uppercase text-rose-700">Chi</p>
                    <p class="mt-0.5 font-mono text-xs font-bold text-rose-800">-{{ formatVnd(last30Days.totalDebitVnd) }}</p>
                </div>
                <div class="rounded-xl bg-stone-100 p-2">
                    <p class="text-[10px] font-semibold uppercase text-stone-700">Ròng</p>
                    <p
                        class="mt-0.5 font-mono text-xs font-bold"
        ...
        <section v-if="creditRows.length" class="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
            <h3 class="text-xs font-semibold uppercase tracking-wide text-emerald-700">Nguồn nạp</h3>
            <ul class="mt-2 divide-y divide-stone-100">
                <li
                    v-for="row in creditRows"
                    :key="`credit-${row.source}`"
                    class="flex items-center justify-between gap-2 py-2"
                >
                    <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-stone-800">{{ row.source_label }}</p>
                        <p class="text-[11px] text-stone-500">{{ row.count }} giao dịch · {{ percent(row.total_vnd, totals.totalCreditVnd) }}</p>
                    </div>
                    <p class="font-mono text-sm font-bold text-emerald-700">+{{ formatVnd(row.total_vnd) }}</p>
                </li>
            </ul>
        </section>

        <section v-if="debitRows.length" class="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm">
            <h3 class="text-xs font-semibold uppercase tracking-wide text-rose-700">Nguồn chi</h3>
            <ul class="mt-2 divide-y divide-stone-100">
                <li
                    v-for="row in debitRows"
                    :key="`debit-${row.source}`"
                    class="flex items-center justify-between gap-2 py-2"
                >
                    <div class="min-w-0">
                        <p class="truncate text-sm font-semibold text-stone-800">{{ row.source_label }}</p>
                        <p class="text-[11px] text-stone-500">{{ row.count }} giao dịch · {{ percent(row.total_vnd, totals.totalDebitVnd) }}</p>
                    </div>
                    <p class="font-mono text-sm font-bold text-rose-700">-{{ formatVnd(row.total_vnd) }}</p>
                </li>
            </ul>
        </section>
