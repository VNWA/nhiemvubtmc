...
<template>
    ...
    <div class="px-3 pb-8 pt-1">
        ...
        <p class="mb-4 text-center text-sm text-stone-500">Chọn phòng để tham gia theo thời gian thực</p>
        <ul v-if="rooms.length" class="space-y-2">
            <li v-for="r in rooms" :key="r.id">
                <Link
                    :href="SukienEventRoomController.show.url(r.slug)"
                ...
                >
                    <div class="flex items-center gap-3">
                        <div class="flex size-12 shrink-0 items-center justify-center overflow-hidden rounded-full border border-stone-200 bg-stone-100 text-xs text-stone-500">
        ...
        <p v-else class="rounded-2xl border border-dashed border-stone-200 bg-amber-50/50 p-6 text-center text-sm text-stone-600">
            Chưa có sự kiện nào. Vui lòng quay lại sau.
        </p>
        <div class="mt-6 text-center">
            <Link
                href="/"
                class="text-sm text-amber-800 underline decoration-amber-400 underline-offset-2"
            >
                ← Về trang chủ
            </Link>
        </div>
